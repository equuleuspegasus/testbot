...
export class PlaylistItem {
    ...
    static async create(url, originalMessage, msgAuthor) {
        ...
        if (
            ...
            try {
                ...
                console.log(error)
                msgAuthor.send(
                    `couldn't get youtube data from \`${url.href}\`...`
                )
                return null
            }
            ...
            let response = await tryGetUrl(
                `https://api.whyp.it/api/tracks/${slug}`
            )
            if (!response) {
            ...
            let response = await tryGetUrl(
                `http://api.soundcloud.com/resolve.json?url=${encodeURIComponent(
                    url.href
            ...
            if (!response) {
                ...
                msgAuthor.send(
                    `couldn't get soundcloud data from \`${url.href}\`...`
                )
                return null
            }
            let data = response.data
            ...
            while (!metadata && attempts < 3) {
                try {
                    metadata = await parseFromTokenizer(httpTokenizer)
                    break
                } catch (e) {
                    console.log(e)
                    attempts++
            ...
            fileinfo.title =
                ...
                "untitled" //originalMessage.attachments.first().name;
    ...
    toString() {
        switch (this.type) {
            case "youtube":
            case "clyp":
                return `[${this.title}](${this.url.href} '${
                    this.url.href
                }') (${this.msDuration()})`
            case "soundcloud":
            case "whyp":
                return `[${this.artist} - ${this.title}](${this.url.href} '${
                    this.url.href
                }') (${this.msDuration()})`
            case "file":
            default:
